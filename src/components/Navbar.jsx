...
export default function Navbar() {
  ...
  return (
    <nav className="fixed inset-x-0 top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 flex items-center gap-8 h-14 md:h-18">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0 select-none">
          ...
          <NavLink to="/" onClick={scrollTop}
            className="text-[1.15rem] md:text-[1.4rem] font-black tracking-wide whitespace-nowrap text-gray-900 hover:text-forest-700 transition-colors">
      ...
      {/* Mobile drawer */}
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ${open ? 'max-h-[40rem] opacity-100' : 'max-h-0 opacity-0'}`}
