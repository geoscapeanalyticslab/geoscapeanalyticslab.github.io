  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          access_key: '86dae9f7-b391-4d34-b709-cc1531103bdd',
          name: form.name,
          email: form.email,
          subject: form.subject || 'New message from GSAL website',
          message: form.message,
          from_name: 'GSAL Website',
        }),
      })
            ...
            <div className="mt-4 space-y-5">
              {[
                ...
                },
                {
                  icon: Clock,
                  title: 'Office Hours',
                  body: 'Monday – Friday: 9:00 AM – 4:00 PM PKT',
                },
          ...
          <ScrollReveal delay={0.1}>
            ...
            {sent ? (
              <div className="bg-forest-50 border border-forest-200 rounded-2xl p-10 text-center">
                ...
                <h3 className="font-black text-forest-900 text-xl">Message Sent</h3>
                <p className="text-forest-700 text-sm mt-2 leading-relaxed max-w-xs mx-auto">
                  Thanks for reaching out! Your message has been delivered to our team — we'll get back to you soon.
                </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-5">
                  <Field label="Full Name" required>
                    <input type="text" required value={form.name} onChange={set('name')}
                      placeholder="Muhammad Mehdi" className={inputCls} />
                  </Field>
                  <Field label="Email" required>
                    <input type="email" required value={form.email} onChange={set('email')}
                      placeholder="bilal@university.edu" className={inputCls} />
                  </Field>
                </div>
                <Field label="Subject">
                ...
                <button type="submit" disabled={sending}
                  className="w-full py-3.5 bg-forest-900 hover:bg-forest-800 text-white font-bold rounded-xl transition-all text-sm uppercase tracking-wide flex items-center justify-center gap-2 disabled:opacity-60">
